"""Generate Gold Lite benchmark result tables and figures."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--summary-dir",
        type=Path,
        default=Path("runs/benchmark/lite"),
...
def load_summaries(summary_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    if missing:
        ...
        raise FileNotFoundError(f"Missing lite summary files:\n{missing_list}")
    return summaries
...
\bottomrule
\end{{tabular}}
\caption{{Test-set performance on the lite tier.}}
\label{{tab:lite_main_results}}
\end{{table}}
"""
...
def build_horizon_table(summaries: dict[str, dict[str, Any]]) -> str:
    return build_breakdown_table(
        ...
        caption="Mean test-set MAE by prediction horizon on the lite tier. Bins are expressed in minutes.",
        label="tab:lite_horizon_mae",
    )
...
def build_delay_delta_table(summaries: dict[str, dict[str, Any]]) -> str:
    return build_breakdown_table(
        ...
        caption=(
            "Mean test-set MAE by delay-delta bin on the lite tier. "
            "Bins are expressed in minutes; negative values correspond to delay "
            "recovery and positive values to delay accumulation."
        ),
        label="tab:lite_delay_delta_mae",
    )


def build_main_df(summaries: dict[str, dict[str, Any]]) -> pd.DataFrame:
    rows = []
    for model in MODEL_ORDER:
...
def build_breakdown_df(
    ...
    for model in MODEL_ORDER:
        ...
        for bucket in bucket_order:
            row[labels[bucket]] = float(summaries[model][key][bucket]["mae"]["mean"])
        rows.append(row)
    return pd.DataFrame(rows)
...
def main() -> None:
    ...
    table_dir = args.output_dir / "tables" / "lite"
    figure_dir = args.output_dir / "figures" / "lite"
    table_dir.mkdir(parents=True, exist_ok=True)
    figure_dir.mkdir(parents=True, exist_ok=True)

    write_text(table_dir / "lite_main_results_table.tex", build_main_table(summaries))
    write_text(table_dir / "lite_horizon_mae_table.tex", build_horizon_table(summaries))
    write_text(table_dir / "lite_delay_delta_mae_table.tex", build_delay_delta_table(summaries))

    configure_plots()
    lite_main = build_main_df(summaries)
    lite_horizon = build_breakdown_df(
        summaries,
        key="per_horizon_minutes",
        bucket_order=HORIZON_ORDER,
        labels=HORIZON_LABELS,
    )
    lite_delay_delta = build_breakdown_df(
        summaries,
        key="per_delay_delta_bin",
        bucket_order=DELAY_DELTA_ORDER,
        labels=DELAY_DELTA_LABELS,
    )
    plot_main_results(lite_main, "lite", figure_dir)
