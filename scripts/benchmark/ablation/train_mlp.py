"""Train one MLP feature-family ablation run.

This script uses the existing tabular Gold dataset, removes one configured
feature family in memory, and trains the same MLP architecture used by the
benchmark. Targets remain in the model training space during optimization; the
validation helper converts them back to seconds when reporting validation MAE.
"""
