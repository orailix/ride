"""Run Optuna hyperparameter search workers for the Transformer benchmark model.

Each trial launches the regular Transformer training script in a per-trial
directory, records the sampled hyperparameters and worker metadata, and reports
the best validation MAE in seconds back to a shared Optuna study. Multiple
workers can share the same SQLite study during Slurm runs.
"""
...
def suggest_hparams(trial: optuna.Trial, args: argparse.Namespace) -> dict[str, int | float | str | bool]:
    """Sample and validate the Transformer training hyperparameters for one trial."""
...
def build_train_command(
    *,
    ...
    """Build the CLI command for the regular Transformer training entry point."""
